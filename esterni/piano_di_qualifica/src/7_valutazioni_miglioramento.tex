...
Il gruppo si è riunito e ha discusso delle problematiche segnalate nella valutazione dei documenti consegnati alla RP. Gli errori più gravi hanno riguardato segnalazioni ricevute in precedenza, che nonostante le soluzioni adottate non avevano risolto i problemi riscontrati, indice che il gruppo non aveva compreso a pieno le osservazioni o che aveva adottato delle soluzioni sbagliate.

Dopo aver discusso delle possibili soluzioni e averle trovate si è deciso di chiedere direttamente ai professori una conferma, per evitare altre incomprensioni o interpretazioni sbagliate.
...
\begin{center}
	\begin{longtable}{|p{4cm}|p{11cm}|}
		...
		Versionamento & Associare ogni azione assegnata a un individuo (creazione, modifica, rimozione) alla corrispondente verifica (anch'essa associata a un altro individuo), se la verifica approva, allora lo stato del CI cambia e con esso il numero di versione; altrimenti l'azione fallisce e il CI non cambia. \\
		Modello di sviluppo & Fare affidamento più su una pianificazione basata sugli obiettivi \\
		Consuntivo di periodo & Ragionare più sulla pianificazione oraria futura in base alle esigenze che si pensa possano essere più utili per il futuro sulla base dell'esperienza attuale e sullo stato del prodotto. \\
		Cruscotto di valutazione & Aggiungere informazioni sugli andamenti globali delle varie fasi e graficare i dati in modo da renderli facilmente fruibili. \\
		Struttura verbali & Adottare un sistema di tracciamento delle decisioni prese in una tabella a fine di ogni verbale. \\
		Quantità verbali & Fare almeno un incontro con il proponente in modo da coinvolgerlo e aggiornarlo sullo stato dello sviluppo. \\
		Processi nelle Norme & Riferiemnto standard iso iec 12207, vari processi presentati in quel doc e si sono elencate le liste di ciò che abbiamo fatto noi. Da standard di processo a processo definito, applicare meglio standard \\
		Attuazione dei rischi & Descrivere più in dettaglio il processo di mitigazione del problema che si è verificato in modo che venga risolto nell'immediato, ma soprattutto, che guardi al futuro e, quindi, che il medesimo problema non si presenti più grazie alla mitigazione adottata nel presente. \\
		Glossario & Correggere i bookmarks mancanti. \\
		Errori in AR & Risolvere gli errori segnalati. \\
