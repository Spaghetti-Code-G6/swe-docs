...
\label{par:verifica}
\subparagraph{Scopo}
\label{par:verifica:scopo}
In questa attività si definiscono le criticità del processo di verifica e si stabilisce come svolgere al meglio il compito di verifica\\
\subparagraph{Compiti}
\label{par:verifica:compiti}
Questa attività è composta dai seguenti compiti:
\begin{itemize}
    \item \textbf{Verifica del contratto}: si verifica di essere in grado di soddisfare i requisiti richiesti e di riuscire a coprire le necessità dell'utente;
    \item \textbf{Verifica di processo}: si verifica che la pianificazione sia accurata, che i processi eseguano nei tempi stabiliti, che le procedure adottate siano efficaci;
    \item \textbf{Verifica dei requisiti}: il software rispetta i requisiti richiesti dal contratto;
    \item \textbf{Verifica del design}: il design implementa la corretta logica di funzionamento del software.
    \item \textbf{Verifica del codice}: il software implementa correttamente ed in maniera tracciata i requisiti.
    \item \textbf{Verifica dell'integrazione}: i vari componnti software interagiscono tra loro in maniera corretta.
    \item \textbf{Verifica della documentazione}:la documentazione è completa, consistente e consegnata in tempo.
\end{itemize}
