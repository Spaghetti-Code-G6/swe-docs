...
È redatta dai \emph{Progettisti}, dovrà includere:
\begin{itemize}
\item Diagrammi UML: diagrammi di classe, diagrammi di package, diagrammi di attività, diagrammi di sequenza.
\item Tecnologie utilizzate: dovranno essere descritte le tecnologie adottate specificandone l’utilizzo del progetto, i vantaggi e gli svantaggi.
\item Design pattern: devono essere descritti i design pattern utilizzati per realizzare l’architettura. Ogni design pattern deve essere accompagnato da una descrizione e un diagramma che ne esponga significato e struttura.
\item Tracciamento delle componenti: ogni requisito deve riferirsi al componente che lo soddisfa.
\item Test di integrazione: l’unione delle parti, intese come classi di verifica, permette di verificare che ogni componente del sistema funzioni nella maniera attesa.
\end{itemize}

\subparagraph{Product Baseline}
È a carico dei \emph{Progettisti}, dovrà presentare:
\begin{itemize}
\item Definizione delle classi: ogni classe deve essere descritta e presentarne scopo e funzionalità, evitando ridondanze. Bisognerà indicarne nome, visibilità, attributi, metodi, breve descrizione che include scopo e funzionalità.
\item Test di unità: devono essere definiti al fine di verificare che le parti funzionino individualmente nel modo stabilito. 
\end{itemize}
...
utile.
L’utilizzo di design pattern permetterà di realizzare codice maggiormente leggibile, semplificherà l’individuazione e correzione di errori, permetterà di realizzare prototipi. \\
...
\item evitare modi che ai servizi o erti dalle librerie esterne utilizzate; 
\item utilizzare ove possibile interfacce e classi astratte; 
\item utilizzare nomi signi cativi per le classi e i metodi in esse contenuti, in modo da facilitare la comprensione immediata di ogni componente; 
